...
import { IconMenu, IconDelete, IconArrowUp, IconArrowDown, IconPlus } from "@arco-design/web-vue/es/icon"
...
const useKey = (schema: Schema) => {
  ...
  return {
    ...
    getKey: (record: any, index?: number) => {
      ...
      if (!(keyMap as any)[index as any]) {
        (keyMap as any)[index as any] = uid()
      }

      return `${(keyMap as any)[index as any]}-${index}`
    },
  }
}
  ...
  setup(props: IArrayBaseAdditionProps, { attrs }) {
    ...
    return () => {
      ...
      return h(
        ...
        {
          default: () => [h(IconPlus), self.value.title || props.title],
        }
      )
    }
  },
